# **RFC4 for Presto: Constant Folding via Expression Evaluation to the Presto Sidecar Process**
...
Constant folding is a technique used in compilers to evaluate constant expressions at compile time. With the advent of Prestissimo, there is now a forked expression evaluation logic, and as of now it is required to duplicate the implementations of all custom C++ functions with identical Java functions. This RFC proposes unify the expression evaluation logic into C++ to address problems created by this fork. This will allow Presto to evaluate constant expressions at query planning time using the same expression evaluation engine used by the evaluation engine, in particular for constant folding, which will reduce the amount of work that needs to be done at query execution time.
...
A new class, `DelegatingRowExpressionInterpreter` will be developed.  It will serve the same purpose as the existing `RowExpressionInterpreter`, except it will function differently.
...
![before.png](RFC-0004%2Fbefore.png)
...
![identified.png](RFC-0004%2Fidentified.png)
...
![simplified.png](RFC-0004%2Fsimplified.png)
